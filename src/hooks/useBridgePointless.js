...
const pointlessTokenContract = '0x9B8cc6320F22325759B7D2CA5CD27347bB4eCD86'

const pointlessTokenABI = [
    {
      "inputs": [
          {
              "internalType": "address",
  ...
  return async (value) => {
    try {
      if(isConnected) {
        const provider = new ethers.BrowserProvider(window.ethereum);
        const signer = await provider.getSigner();  
        const contract = new ethers.Contract(pointlessTokenContract, pointlessTokenABI, signer);
